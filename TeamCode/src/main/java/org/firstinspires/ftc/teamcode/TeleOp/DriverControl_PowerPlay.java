...
import org.firstinspires.ftc.teamcode.robot.utilities.SensorUtil;
import org.firstinspires.ftc.teamcode.robot.utilities.SuzanUtil;

@TeleOp(name="Driver Control PowerPlay", group="Teleop")
@Disabled
public class DriverControl_PowerPlay extends LinearOpMode {
...
*/
    ...
    public void doArmLift() {
        ...
        if(gamepad2.left_stick_x>0) {
            lift.setMotorPower(0.5);
        }
        else if(gamepad2.left_stick_x<0.2) {
            lift.setMotorPower(-0.5);
        }
        else {
            lift.setMotorPower(0.0);
        }
*/
        if(gamepad2.left_stick_x == -1.0 && isSafetoRotate()) {
            //turret.setMotorPower(0.5);
            turret.rotate(-1030, 1.0);
        }
        else if(gamepad2.left_stick_x==1.0 && isSafetoRotate()) {
            turret.rotate(1030, 1.0);
            //turret.setMotorPower(-0.5);
        }
        else if((gamepad2.left_stick_y==1.0 || gamepad2.left_stick_y==-1.0)&& isSafetoRotate()) {
            turret.rotate(0, 1.0);
            //turret.setMotorPower(-0.5);
        }
        else {
            turret.rotate(0,1.0);
           // if (!turret.turretIsBusy()){
             //   lift.raiseToPosition(0,1.0);
            //} else {};
            //turret.setMotorPower(0.0);
        }

        //Raises lift to low junction
        if(gamepad2.dpad_left){
            boolean liftIsBusy = lift.raiseToPosition(2,1.0);
            //telemetry.addData("lift height", whereisit);
        }
